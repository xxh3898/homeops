...
assert_nginx_location_contains \
  "${NGINX_CONFIG}" "/api/v1/internal/agent/control-results" \
  'proxy_set_header X-HomeOps-Agent-Verified $ssl_client_verify;'
assert_contains "${NGINX_CONFIG}" 'location = /api/v1/internal/agent/recovery-requests/next {'
assert_contains "${NGINX_CONFIG}" 'location = /api/v1/internal/agent/recovery-results {'
assert_nginx_location_contains \
  "${NGINX_CONFIG}" "/api/v1/internal/agent/recovery-requests/next" \
  'limit_except GET { deny all; }'
assert_nginx_location_contains \
  "${NGINX_CONFIG}" "/api/v1/internal/agent/recovery-requests/next" \
  'proxy_read_timeout 5s;'
assert_nginx_location_contains \
  ...
  'proxy_set_header X-HomeOps-Agent-Verified $ssl_client_verify;'
assert_nginx_location_contains \
  "${NGINX_CONFIG}" "/api/v1/internal/agent/recovery-results" \
  'limit_except POST { deny all; }'
assert_nginx_location_contains \
  "${NGINX_CONFIG}" "/api/v1/internal/agent/recovery-results" \
  'client_max_body_size 4k;'
