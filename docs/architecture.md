...
Activity는 `deployment`, `backup_run`, `incident`, `agent_event`, `container_action_audit` 다섯 source를 PostgreSQL visibility snapshot으로 projection합니다. [ADR-001](adr/ADR-001-operational-history-retention-and-deletion-safety.md)은 production aggregate evidence에서 cleanup benefit이 입증되지 않은 `deployment`, `backup_run`, `agent_event`를 current `KEEP_UNBOUNDED` operating decision으로 두고, `incident`를 별도 prerequisite 전까지 finite candidate에서 제외하며 privileged `container_action_audit`를 별도 Security/Audit Decision으로 분리합니다. 이는 영구 보존 Product requirement가 아니라 evidence 없이 destructive runtime과 임의 duration을 만들지 않는 fail-closed boundary입니다.
...
V12 `ingestion_event_key_ledger`는 deployment/backup business history와 분리된 source-scoped replay authority이며 DB-level business INSERT fence가 old/new application writer 모두에 ledger reservation을 강제합니다. V13은 같은 minimal ledger에 `SIGNAL` namespace와 signal-event INSERT fence를 additive로 연결해 episode history와 key replay authority를 분리합니다. Business/event table의 unique `event_key`는 defense-in-depth로 유지하고 ledger cleanup은 disabled입니다. Incident는 notification FK/provenance와 연결됩니다. Cursor TTL과 retention duration의 단순 비교는 deletion ordering의 증명이 아닙니다. Existing metric aggregate, processed snapshot ledger, health-check result와 terminal notification retention의 기간·cleanup semantics는 이 결정으로 바뀌지 않습니다.
