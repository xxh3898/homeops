...
현재 Activity cursor는 visibility snapshot과 filter scope를 보존하지만 bounded expiry가 없습니다. Deployment/backup business row는 `event_key` replay barrier도 겸하고, incident는 notification FK/provenance와 연결됩니다. 따라서 finite retention은 cursor validity, 별도 ingestion idempotency authority, dependency-safe incident policy와 recovery gate를 먼저 요구합니다. Existing metric aggregate, processed snapshot ledger, health-check result와 terminal notification retention의 기간·cleanup semantics는 이 결정으로 바뀌지 않습니다.
