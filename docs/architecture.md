...
Public control POST는 ADMIN session과 기존 CSRF 외에도 exact HTTPS Origin/Host/proxy contract, fixed operation·confirmation과 canonical UUID idempotency key를 요구합니다. Idempotency winner의 V10 audit reservation transaction이 commit된 뒤에만 broker enqueue가 가능하며 POST는 Docker completion을 기다리지 않습니다. Broker result는 별도 bounded executor에서 `REQUESTED` 행에 compare-and-set으로 투영하고, crash로 completion path를 잃은 오래된 public reservation은 operation TTL과 reporting grace 뒤 `OUTCOME_UNKNOWN/RESULT_UNAVAILABLE`로만 복구합니다. Polling response에는 operation UUID, 12자리 ID, operation, stable status/reason과 timestamp만 포함합니다.
