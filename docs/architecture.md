...
Rhaomi automatic recovery는 generic container control과 분리된 server-owned decision domain입니다. Actual incident OPEN winner가 V14의 exact `rhaomi/(rhaomi-web|backend)` mapping을 잠그고, row가 존재하며 enabled이고 target의 마지막 authority reservation에서 30분이 지났을 때만 incident-unique `REQUESTED` audit과 cooldown timestamp를 같은 transaction에 기록합니다. Dispatcher는 incident active 상태, 현재 mapping과 fresh `supportsRhaomiRecovery` Agent capability를 다시 확인한 뒤 work를 `DISPATCHED`로 compare-and-set하고, UUID·project·target·`RESTART`·10초 start expiry만 bounded broker에 전달합니다. Agent는 local compile-time fixed inventory에서 entrypoint를 resolve해 shell 없이 고정 environment로 최대 한 번 실행하고 최대 4 KiB JSON만 decode합니다. Claimed work, failure 또는 `OUTCOME_UNKNOWN`은 자동 requeue하지 않습니다. Generic control의 managed/project/mount protection은 바뀌지 않으며 writable media bind가 있는 Rhaomi backend는 계속 generic control deny 대상입니다.
...
HomeOps는 전용 PostgreSQL instance와 volume을 사용하며 다른 프로젝트와 데이터베이스를 공유하지 않습니다. 현재 구현은 1분 host metric aggregate, Agent liveness, 범위가 제한된 처리 snapshot 멱등성 ledger를 저장하고, 최신 상세 container snapshot은 API memory에 둡니다. container inventory response에는 Agent freshness가 포함되므로 Agent가 멈춰도 오래된 RUNNING 또는 HEALTHY 상태가 최신처럼 표시되지 않습니다. Bounded monitoring signal은 every-observation row 대신 episode당 최대 한 ALERT와 한 RECOVERED event를 저장하고, fake service 없이 nullable-service incident 하나에 연결합니다. `(project, signal_type)` active partial unique index와 episode/status unique event가 concurrent caller를 한 winner로 수렴시킵니다. V14 automatic recovery mapping은 service와 exact logical target을 일대일로 묶고 durable `last_reserved_at`을 소유하며, attempt는 incident unique와 status별 stable reason constraint로 requested/skipped/terminal evidence만 저장합니다. Raw command, output, path, URL, full container ID와 content data는 저장하지 않습니다.
...
Activity는 `deployment`, `backup_run`, `incident`, `agent_event`, `container_action_audit`, `automatic_recovery_attempt` 여섯 source를 PostgreSQL visibility snapshot으로 projection합니다. Recovery row는 기존 public `CONTAINER_ACTION` type 아래 deterministic title/status, requested timestamp와 `rhaomi/<target>` 또는 `unmapped` context만 노출하며 stable reason과 detailed health evidence는 audit table에만 둡니다. [ADR-001](adr/ADR-001-operational-history-retention-and-deletion-safety.md)은 production aggregate evidence에서 cleanup benefit이 입증되지 않은 `deployment`, `backup_run`, `agent_event`를 current `KEEP_UNBOUNDED` operating decision으로 두고, `incident`를 별도 prerequisite 전까지 finite candidate에서 제외하며 privileged control/recovery audit를 별도 Security/Audit Decision으로 분리합니다. 이는 영구 보존 Product requirement가 아니라 evidence 없이 destructive runtime과 임의 duration을 만들지 않는 fail-closed boundary입니다.

Activity cursor는 visibility snapshot과 filter scope를 versioned HMAC token에 묶고, first-page `snapshotAt` 기준 최대 1시간의 server-enforced validity를 적용합니다. Process-local signing key는 restart 때 rotate되며 unsigned legacy 또는 변조한 cursor는 store query 전에 fail closed합니다. Future finite source의 physical age authority는 server-owned `recorded_at`이고 display/order는 domain event time을 유지합니다. Duration `D`가 승인되면 logical cutoff는 최초 `snapshotAt - D`로 고정하며 continuation마다 현재 시각으로 다시 계산하지 않습니다. Active/nonterminal row는 age만으로 숨기지 않습니다. Logical exclusion 뒤 해당 row를 볼 수 있던 모든 cursor가 expire 또는 deterministic invalid 처리되고 source prerequisite가 충족된 뒤에만 physical deletion candidate가 될 수 있습니다.

V12 `ingestion_event_key_ledger`는 deployment/backup business history와 분리된 source-scoped replay authority이며 DB-level business INSERT fence가 old/new application writer 모두에 ledger reservation을 강제합니다. V13은 같은 minimal ledger에 `SIGNAL` namespace와 signal-event INSERT fence를 additive로 연결해 episode history와 key replay authority를 분리합니다. V14는 default-none recovery mapping과 incident-unique bounded attempt를 additive로 추가하며 기존 operational history를 수정하지 않습니다. Business/event table의 unique `event_key`는 defense-in-depth로 유지하고 ledger cleanup은 disabled입니다. Incident는 notification과 recovery provenance에 연결됩니다. Cursor TTL과 retention duration의 단순 비교는 deletion ordering의 증명이 아닙니다. Existing metric aggregate, processed snapshot ledger, health-check result와 terminal notification retention의 기간·cleanup semantics는 이 결정으로 바뀌지 않습니다.

HomeOps는 container log, Docker inspect document, `.env` 내용, credential, webhook URL을 저장하지 않습니다. `backup_run`은 다른 프로젝트의 backup 결과를 설명할 뿐 HomeOps가 자체 데이터베이스를 자동 백업한다는 뜻이 아닙니다. 신뢰하는 script는 범위가 제한된 시간 HMAC signature가 있을 때만 deployment, backup 또는 allowlisted signal lifecycle을 제출할 수 있습니다. Source-scoped ledger key는 재사용을 막고 current business/event row는 digest·lifecycle의 detailed authority이므로 exact retry는 멱등하고 terminal state 변경은 거부됩니다. Signal persistence는 project, enum type/status, timestamp와 bounded numeric measurement만 저장하며 raw payload/URL/path/log를 위한 JSONB bag을 두지 않습니다. Ledger에는 payload, digest, business UUID 또는 private metadata를 복제하지 않습니다. service-check 대상은 운영자가 제공한 정확한 HTTPS origin과 일치해야 하고 redirect를 따르지 않으며 request timeout은 제한됩니다.
