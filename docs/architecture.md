...
현재 Activity cursor는 visibility snapshot과 filter scope를 보존하지만 bounded expiry가 없습니다. Deployment/backup business row는 `event_key` replay barrier도 겸하고, incident는 notification FK/provenance와 연결됩니다. 따라서 finite retention은 logical visibility cutoff, cursor validity/invalidation boundary와 physical deletion eligibility를 분리해야 하며, server가 아직 수락할 cursor가 참조할 수 있는 row를 hard-delete하면 안 됩니다. Cursor TTL과 retention duration의 단순 비교는 이 ordering의 증명이 아닙니다. 별도 ingestion idempotency authority, dependency-safe incident policy와 recovery gate도 먼저 충족해야 합니다. Existing metric aggregate, processed snapshot ledger, health-check result와 terminal notification retention의 기간·cleanup semantics는 이 결정으로 바뀌지 않습니다.
