...
현재 production에서 사용자에게 제공하는 호스트와 컨테이너 기능은 읽기 전용입니다. 호스트·컨테이너 인벤토리, bounded metric history, freshness-aware Container Detail, explicit opt-in bounded/redacted Container Logs, HMAC 인증 배포/백업 수집, 정확한 origin HTTP 서비스 점검, 인시던트 상태 전환, 범위 제한 점검 결과 보존, 페이지네이션 Activity 타임라인을 구현합니다. 운영 이력 입력은 운영자가 관리하는 secret 또는 origin allowlist를 설정하기 전까지 fail closed하고, Container Logs는 fresh capability와 container별 exact opt-in이 없으면 fail closed합니다. Phase 4의 transactional Discord outbox, fail-closed service eligibility authority와 deployment·backup·incident·Agent lifecycle·Docker episode producer도 production acceptance를 완료했습니다. Acceptance 종료 뒤 webhook Secret은 설치된 상태로 유지하되 `HOMEOPS_NOTIFICATIONS_ENABLED=false`로 outbound를 닫아 두었습니다. Phase 5 source에는 fixed Agent control protocol, ADMIN 전용 public mutation/polling API와 durable audit 및 Container Detail control UI가 있습니다. Production V10 migration, Agent rollout, allowlist·managed-label activation과 실제 control acceptance는 별도 release gate로 남아 있습니다.
...
Public control POST는 ADMIN session과 기존 CSRF 외에도 exact HTTPS Origin/Host/proxy contract, fixed operation·confirmation과 canonical UUID idempotency key를 요구합니다. Existing durable key는 exact replay/conflict로 먼저 판정하고, genuinely new key만 principal+key 5/60초 limiter를 통과한 뒤 V10 audit reservation을 commit합니다. Durable created winner의 commit 후에만 broker enqueue가 가능하며 POST는 Docker completion을 기다리지 않습니다. Broker result는 별도 bounded executor에서 `REQUESTED` 행에 compare-and-set으로 투영하고, crash로 completion path를 잃은 오래된 public reservation은 operation TTL과 reporting grace 뒤 `OUTCOME_UNKNOWN/RESULT_UNAVAILABLE`로만 복구합니다. Polling response에는 operation UUID, 12자리 ID, operation, stable status/reason과 timestamp만 포함합니다.
