...
Phase 5는 Source IMPLEMENTED / Production INACTIVE / Acceptance NOT DONE입니다. `HOMEOPS_CONTROL_ALLOWED_PROJECTS`는 기본 empty이며 exact `homeops.managed=true`, fresh snapshot, unique short-ID match와 exact allowlisted non-HomeOps project가 모두 있어야 work를 enqueue할 수 있습니다. Public ADMIN API는 CSRF, exact HTTPS Origin/Host, exact confirmation과 canonical UUID idempotency key를 요구합니다. Existing durable key replay/conflict는 limiter 앞에서 결정하고, genuinely new key만 principal+key 5/60초 bound를 통과한 뒤 V10 audit reservation을 commit하며 durable created winner만 existing broker를 호출합니다. Audit에는 fixed operation, 12자리 ID, principal, stable status/reason과 timestamp만 저장하며 image, confirmation, raw error와 private metadata는 저장하지 않습니다. Result는 async CAS로 반영하고 operation TTL·reporting grace 이후 남은 public `REQUESTED`는 requeue 없이 `OUTCOME_UNKNOWN`으로 복구합니다. Agent outbound worker는 live label/project/service/mount를 재검증하고 protected/stateful target을 hard deny한 뒤 fixed `START|STOP|RESTART` Docker POST만 work당 최대 한 번 수행합니다. Backend active work는 global 1개이며 PENDING operation deadline은 15초, CLAIMED result-reporting grace는 추가 fixed 15초입니다. Container Detail UI는 별도 confirmation, one-key ambiguous retry와 GET-only bounded status polling을 사용하며 UI candidate gating을 final authority로 취급하지 않습니다. Production V10 migration, application release, persistent Agent artifact publication/rollout, production allowlist/label activation과 실제 control acceptance는 각각 별도 gate이므로 source merge를 운영 mutation 권한으로 해석하지 마세요.
