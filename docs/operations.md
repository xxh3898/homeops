...
Phase 3 완료 뒤 추가된 bounded signal source는 `DISK_LOW`, `HTTP_5XX_BURST`의 logical `ALERT`/`RECOVERED` episode만 받아 기존 incident/Activity에 연결합니다. Every-observation history, arbitrary signal/URL/JSON, raw log와 private endpoint는 받지 않습니다. Main release `950a0ec47ee7099fc7d525838748b06a96121c61`에서 Flyway V13, installed reporter의 fixed `signal` mode, 두 lifecycle, idempotent replay와 Activity privacy를 production에서 검증했습니다. Acceptance 뒤에도 `HOMEOPS_NOTIFICATIONS_ENABLED=false`이며 signal canary가 notification intent나 outbound를 만들지 않았습니다. Old API의 `/signals` 404/405 동안 reporter spool은 event를 삭제하거나 quarantine하지 않고 bounded retry 상태로 보존합니다. 이후 caller 추가 또는 notification authority 변경은 별도 production gate입니다.
...
Operational Activity history 삭제 runtime은 현재 구현되지 않았고 production activation도 승인되지 않았습니다. Read-only aggregate evidence에서 candidate source의 storage/query/privacy pressure와 duration 근거가 확인되지 않았으므로 `deployment`, `backup_run`, `agent_event`는 current `KEEP_UNBOUNDED`입니다. 관계 크기나 query degradation이 material해지거나 explicit privacy requirement 또는 accepted recovery capability가 생길 때만 source 하나를 새 evidence로 재평가합니다. 단순 row 증가나 임의 기간은 구현 trigger가 아닙니다. Incident/notification dependency와 privileged audit 경계는 계속 별도 Decision입니다.

Future finite source의 physical age authority는 server-owned `recorded_at`이고 Activity display/order는 domain event time을 유지합니다. 첫 page에서 `logicalCutoff = cursorChain.snapshotAt - D`를 고정해 continuation 전체에 재사용하며 active/nonterminal deployment·backup row는 age만으로 숨기거나 삭제하지 않습니다. Logical cutoff를 지난 뒤에도 server가 수락할 cursor가 row를 참조할 수 있으면 hard-delete할 수 없습니다. 해당 cursor가 expire 또는 deterministic invalid 처리되고 source-specific idempotency/dependency/recovery prerequisite까지 통과한 뒤에만 physical deletion candidate가 됩니다. Cursor TTL과 retention duration의 단순 비교는 이 ordering을 증명하지 않습니다. V12 ledger cleanup은 disabled 상태를 유지합니다.
...
| Rhaomi automatic recovery `FAILED` 또는 `OUTCOME_UNKNOWN` | 같은 incident를 requeue하거나 capability를 다시 실행하지 않고 bounded audit 보존 | incident, mapping/cooldown, fresh Agent capability와 fixed recovery lock·identity evidence를 read-only로 분리 점검. 반복 restart, lock 삭제 또는 service mutation은 별도 Rhaomi Ops 승인 필요 |

Production worker는 ingestion ledger backfill을 포함한 migration 전에 current API를 완전히 정지하고, migration 성공 뒤에만 candidate API/Web을 활성화합니다. Migration 실패 시 accepted deployment state와 runtime-config current pointer를 candidate로 바꾸지 않습니다. V12 DB는 old/new application writer 모두의 deployment/backup INSERT 전에 ledger reservation을 강제하므로 migration 성공 뒤 candidate verification rollback에서도 ledgerless business row를 허용하지 않습니다. V14는 default-none mapping과 additive audit table만 추가하지만 적용 자체는 별도 production migration gate입니다. 이미 적용한 V14 table을 image rollback 과정에서 drop하거나 attempt evidence를 지우지 말고 mapping 부재 또는 disabled 상태를 유지하세요. 다른 migration은 이미 database를 바꿨을 수 있으므로 image rollback이 backward-compatible할 때만 안전합니다. 자동 path에 incompatible migration을 사용하지 마세요.
...
- Rhaomi recovery mapping의 default-none/disabled 상태, durable cooldown과 bounded attempt audit, fresh Agent capability
...
Rhaomi automatic recovery는 Source IMPLEMENTED / Production INACTIVE / Acceptance NOT RUN입니다. Actual incident OPEN winner가 exact mapping과 target row를 잠근 뒤 row 존재, enabled와 durable 30분 cooldown을 만족할 때만 incident-unique request를 예약합니다. Dispatcher는 active incident, 현재 mapping과 fresh `supportsRhaomiRecovery` capability를 다시 확인하고 bounded work를 최대 한 번 전달합니다. Agent는 fixed inventory에서 exact `restart rhaomi-web|backend`만 shell 없이 실행하며 failure, timeout, invalid result와 unknown outcome을 자동 재실행하지 않습니다. Generic control의 writable-mount deny는 바뀌지 않습니다.

Production activation은 V14 적용, exact monitored-service/mapping 등록, enabled 전환, fixed recovery artifact 설치 검증, controlled Agent rollout, rollback과 observation window를 포함한 별도 Ops 계획이 필요합니다. Source merge, application deploy 또는 Agent artifact publish만으로 mapping을 만들거나 enable하지 않습니다. Acceptance 전에는 실제 Rhaomi restart를 유도하지 않으며 mapping row 부재 또는 disabled 상태를 유지합니다. `FAILED`나 `OUTCOME_UNKNOWN` 뒤 같은 incident를 수동으로 반복 실행하거나 preserved recovery lock을 삭제하지 말고 먼저 incident, audit와 Rhaomi capability evidence를 읽기 전용으로 확인하세요. Mapping/attempt 삭제도 incident 대응 절차가 아니며 별도 data-retention 결정과 파괴 작업 승인이 필요합니다.
