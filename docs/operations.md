...
Phase 5 control foundation은 Source PARTIAL / Production INACTIVE / Acceptance NOT DONE입니다. `HOMEOPS_CONTROL_ALLOWED_PROJECTS`는 기본 empty이며 exact `homeops.managed=true`, fresh snapshot, unique short-ID match와 exact allowlisted non-HomeOps project가 모두 있어야 internal work를 enqueue할 수 있습니다. Public enqueue/API/UI와 DB audit record는 없습니다. Agent outbound worker는 live label/project/service/mount를 재검증하고 protected/stateful target을 hard deny한 뒤 fixed `START|STOP|RESTART` Docker POST만 work당 최대 한 번 수행합니다. Backend active work는 global 1개이며 PENDING operation deadline은 15초, CLAIMED result-reporting grace는 추가 fixed 15초입니다. Claimed work는 operation deadline에 `EXPIRED`로 축소하거나 requeue하지 않고, grace까지 result가 없으면 `OUTCOME_UNKNOWN`으로 terminalize합니다. Application release, persistent Agent artifact publication/rollout, production allowlist/label activation과 실제 control acceptance는 각각 별도 gate이므로 source merge를 운영 mutation 권한으로 해석하지 마세요.
