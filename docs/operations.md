...
| HomeOps | 신뢰하는 reporter의 future deployment·backup lifecycle, persisted exact-origin incident 중 명시적으로 opt-in한 service의 future transition, API가 살아 있는 동안 expected native Agent의 freshness/version lifecycle | 각 source producer와 Phase 4 Discord delivery가 활성화된 이후 |
...
Deployment와 backup producer는 실제 ingestion insert 또는 terminal transition winner만 typed outbox intent로 기록하며 replay나 경쟁 loser는 새 intent를 만들지 않습니다. Backup payload는 project, database type, status만 허용하고 logical location, failure, expiry와 restore metadata는 제외합니다. Incident producer는 event 시점의 persisted service authority를 확인하고 actual OPEN winner, 기본 15분 이상 지속된 DOWN observation, actual recovery winner만 고려합니다. Agent producer는 persisted expected status row의 last snapshot을 stale episode root로 사용하고 actual current snapshot update winner만 recovery/version intent를 만들며, first connection·duplicate·out-of-order snapshot은 notification을 만들지 않습니다. Incident와 Agent의 recovery는 같은 episode의 SENT root에만 parent로 연결하고 suppressed/pending/failed/unknown/missing root에는 child를 만들지 않습니다. Global switch가 disabled이면 root와 version intent는 `SUPPRESSED`로 끝나고 enable 뒤 재생되지 않습니다. Agent freshness는 `HOMEOPS_AGENT_STALE_AFTER`를 재사용하고 checker cadence만 별도 bounded setting으로 둡니다. Docker는 별도 producer 계약으로 추가합니다. Optional email escalation은 같은 incident의 owner와 duplicate-prevention policy를 별도로 검증한 뒤에만 고려합니다.

Native Agent는 exact `homeops.notifications=true`를 container별 `notificationsAllowed` boolean으로만 전달합니다. 이 capability는 `homeops.managed`와 `homeops.logs`에서 독립적이고 old/rollback Agent가 field를 생략하면 false입니다. 현재 Backend는 future Docker producer가 사용할 수 있도록 latest snapshot 안에만 이 값을 보존하며 public inventory/detail에는 노출하지 않습니다. Docker episode 저장, outbox producer와 Discord delivery는 아직 없으므로 production label opt-in이나 Agent rollout은 별도 gate 없이는 수행하지 않습니다.
