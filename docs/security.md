...
| API outbound egress abuse | low/medium | medium/high | API-only non-internal egress network, exact HTTPS origin allowlist, no general request endpoint | Docker `internal` network attributes, not `ingress`/`egress` names, enforce the outbound boundary; a compromised API process can still initiate outbound traffic |
