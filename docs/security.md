...
| CSRF | 낮음/중간 | 높음 | SameSite session, CSRF token, control POST의 server-owned canonical public HTTPS Origin과 fixed proxy scheme guard | 침해된 허용 ADMIN browser session 또는 host account는 별도 경계가 필요 |
...
| automatic recovery authority 오용 | 낮음/중간 | 높음 | mapping row 부재와 disabled default, incident unique reservation, target row lock과 durable 30분 cooldown, fresh Agent capability, fixed inventory | HomeOps·Agent 또는 승인된 mapping authority가 함께 침해되면 cooldown당 target restart 한 번을 유발할 수 있음 |
...
Phase 5 source는 exact `homeops.managed=true`, fresh latest snapshot, unique bounded short ID와 default-empty exact Compose project allowlist를 함께 요구한 뒤에만 control work를 생성할 수 있습니다. Public POST는 ADMIN + CSRF + server-owned canonical public HTTPS Origin, fixed operation·confirmation과 canonical UUID idempotency key를 요구합니다. Canonical origin은 host와 non-default explicit port를 exact 비교하고 HTTPS default `443`은 canonical no-port와 동일하게 normalize하며 missing/invalid configuration은 fail closed합니다. Nginx가 고정한 HTTPS scheme은 계속 요구하지만 proxy-internal `Host`나 caller-supplied forwarded-host는 public authority로 신뢰하지 않습니다. Existing durable replay/conflict는 rate slot을 소비하지 않고, absent key만 principal+key 5/60초 limiter에서 distinct key로 세며, overflow는 audit INSERT와 broker work 없이 429로 종료합니다. 허용된 key의 durable reservation commit 후 created winner만 bounded broker를 호출하고 global active 1을 유지하며 polling은 bounded audit projection만 노출합니다. Agent는 operation 직전에 current Docker list/inspect에서 unique full ID, exact label/project, nonblank service와 mount protection을 다시 확인합니다. `homeops`, protected database/cache service 이름, writable bind/volume와 판정 불가능 mount는 hard deny합니다. Work는 fixed operation과 expiry 외 command/path/query/body를 받지 않으며 Docker operation은 최대 한 번입니다. Result delivery만 memory에서 재시도하고 ambiguous post-send outcome은 자동 재실행하지 않습니다. Full Docker ID, principal, confirmation, raw labels/mount source/error body, image/registry와 allowlist 전체를 public response, result, error 또는 log에 노출하지 않으며 audit에도 image/raw failure/private metadata를 저장하지 않습니다.

## Rhaomi fixed recovery 경계

Rhaomi automatic recovery는 generic Docker control의 예외가 아니라 별도 fixed capability입니다. Actual incident OPEN winner만 server-owned mapping을 평가하고, incident unique attempt와 exact target mapping row lock으로 경쟁 caller를 한 durable decision에 수렴시킵니다. Mapping row가 없거나 disabled이거나, 30분 cooldown이 남았거나 incident가 더 이상 active이거나 fresh capability가 없으면 mutation work를 만들지 않습니다. Cooldown은 성공이 아니라 authority reservation 때 시작하므로 failure와 unknown outcome 뒤에도 자동 재시도하지 않습니다.

Nginx는 기존 loopback mTLS listener의 exact recovery poll/result path만 GET/POST로 허용하고 result body를 4 KiB로 제한합니다. Agent work에는 UUID, `rhaomi`, `rhaomi-web|backend`, `RESTART`와 짧은 expiry만 있습니다. Executable, path, working directory, environment, Docker argument 또는 caller command를 받지 않습니다. Agent는 local compile-time fixed inventory, fixed locale/PATH와 shell-free execution을 사용하고 stdout은 4 KiB strict JSON만 decode하며 stderr와 raw output을 폐기합니다. Result delivery만 bounded grace 안에서 재시도하고 capability를 다시 실행하지 않습니다. Invalid/oversize result, timeout 또는 restart 결과가 모호하면 `OUTCOME_UNKNOWN`으로 보존합니다.

Generic control의 managed/project/service/mount revalidation은 그대로 유지됩니다. Writable media bind를 가진 Rhaomi backend는 automatic recovery source가 있어도 generic Docker mutation의 deny 대상입니다. Recovery audit에는 incident·logical target, stable status/reason, bounded timestamp·health·restart count만 저장하고 raw command, output, path, URL, full container ID와 content data를 저장하지 않습니다. Activity에는 reason과 detailed evidence를 제외한 deterministic title/status와 `rhaomi/<target>` 또는 `unmapped` context만 노출합니다. Source 구현은 mapping enable, Agent rollout, production restart 또는 acceptance 권한이 아닙니다.
...
- Activity는 deployment, backup, incident, Agent event, container control과 automatic recovery audit의 bounded allowlist context만 노출하며 visibility-snapshot cursor를 사용하는 no-store pagination을 제공합니다. Signal은 fake monitored service 없이 bounded incident title/type로만 같은 projection에 나타납니다. Event/episode key와 numeric measurement는 Activity에 노출하지 않습니다. Control audit에서는 12자리 public container ID만 context로 사용하고 principal, idempotency key, name/image, failure/raw metadata와 reason code를 제외합니다. Automatic recovery도 logical target 외 raw result, reason, path와 output을 제외합니다.
...
- Production V10 migration, Agent rollout, canonical Origin, allowlist/managed-label activation과 controlled control acceptance는 분리된 gate로 완료했습니다. 이후 대상 opt-in이나 authority 변경은 여전히 별도 production 승인을 요구합니다.
- V14 automatic recovery source는 구현됐지만 production mapping은 비활성이고 acceptance는 수행하지 않았습니다. V14 apply, exact mapping/enable, Agent rollout과 실제 recovery drill은 서로 독립된 production gate입니다.
