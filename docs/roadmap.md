...
Phase 3 완료 뒤 source extension으로 `DISK_LOW`, `HTTP_5XX_BURST`의 bounded monitoring episode ingestion을 추가했습니다. Exact HMAC endpoint는 typed `ALERT`/`RECOVERED` request만 받고 durable event-key ledger, active project/type uniqueness와 같은-incident recovery를 사용합니다. Signal incident는 기존 bounded Activity projection을 재사용하지만 notification producer, Production reporter/caller activation과 live acceptance는 포함하지 않습니다. 따라서 기존 Phase 3 완료 상태를 재정의하지 않으며 installed production capability 판정은 별도 Release·V13 migration·activation gate에서 수행합니다.
